...
class TestLocalAgentFlow:
    def test_install_creates_full_agent_structure(self, tmp_path: Path) -> None:
    ...
    def test_local_skill_registered_and_executable(self, tmp_path: Path) -> None:
        git_dir = tmp_path / ".git"
        git_dir.mkdir()
    ...
    def test_version_mismatch_warns_but_does_not_abort(self, tmp_path: Path) -> None:
        git_dir = tmp_path / ".git"
        git_dir.mkdir()
    ...
    def test_legacy_project_validate_and_scan(self, tmp_path: Path) -> None:
        git_dir = tmp_path / ".git"
        git_dir.mkdir()
        ...
        (tests_dir / "integration" / "test_dummy.py").write_text(
            "def test_dummy(): pass\n"
        )
